...
import (
	...
	token "github.com/ssup2-playground/msa-event-market_service-auth/pkg/auth/token"
)
...
func (_m *TokenService) CreateTokens(ctx context.Context, loginID string, passwd string) (*token.TokenInfo, *token.TokenInfo, error) {
	...
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, string, string) (*token.TokenInfo, *token.TokenInfo, error)); ok {
		return rf(ctx, loginID, passwd)
	}
...
func (_m *TokenService) RefreshToken(ctx context.Context, refreshToken string) (*token.TokenInfo, error) {
	...
	var r0 *token.TokenInfo
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (*token.TokenInfo, error)); ok {
